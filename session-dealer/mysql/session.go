...
type MysqlSession struct {
	...
	ignoreAckID                   int64
	sendSize                      int64
	uncompressedExpectReceiveSize int
}
...
func NewMysqlSession(
	...
	ms = &MysqlSession{
		...
		sendSize:                      0,
		pkgCacheLock:                  sync.Mutex{},
		uncompressedExpectReceiveSize: -1,
	}
...
func (ms *MysqlSession) checkFinish() bool {
	...
	if checkNode.end-checkNode.begin == int64(len(ms.cachedStmtBytes)) || ms.uncompressedExpectReceiveSize == len(ms.cachedStmtBytes) {
		return true
	}
...
func (ms *MysqlSession) clear() {
	...
	ms.ignoreAckID = -1
	ms.sendSize = 0
	ms.uncompressedExpectReceiveSize = -1
...
func (ms *MysqlSession) readFromClient(seqID int64, bytes []byte) {
	...
	if ms.expectReceiveSize == -1 {
		...
		if int64(ms.expectReceiveSize) < int64(len(contents)) {
			...
			if uint32(bytes[4]) > 0 {
				//解压后字节长度
				ms.expectReceiveSize = extractMysqlPayloadSize(bytes[4:7])
				//尝试解压
				unData, err := DoZlibUnCompress(contents)
				if err != nil {
					log.Debug("receive invalid mysql packet,decompression failed")
					return
				}
				if int64(ms.expectReceiveSize) < int64(len(unData)) {
					log.Debug("receive invalid mysql packet")
					return
				}
				contentSize = int64(len(contents)) - 4
				contents = unData[4:]
				ms.uncompressedExpectReceiveSize = len(contents)
			} else {
				if int64(ms.expectReceiveSize) < int64(len(contents)) {
					log.Debug("receive invalid mysql packet")
					return
...
func filterQueryPieceBySQL(mqp *model.PooledMysqlQueryPiece, querySQL []byte) *model.PooledMysqlQueryPiece {
	...
	if dmlPatern.Match(querySQL) {
		mqp.SetNeedSyncDml(true)
	}
